...
module.exports = {
    ...
    checkout: async(req, res) => {
        try {
            const { accountUser, name, payment, nominal, voucher, bank} = req.body;
    ...
    historyDetail: async(req, res) => {
        try {
            ...
            if(!history) return res.status(401).json({ message: "data history tidak ditemukan.!" })
    ...
    editProfile: async(req, res, next) => {
        try {
            ...
            if(req.file){
                ...
                const player = await Player.findOneAndUpdate({ _id: req.player._id},
